...
class ArticleVenteController extends Controller
{
    ...
    public function store(Request $request)
    {
        return DB::transaction(function () use ($request) {
            ...
            if (!isset($request->articles) || count($request->articles) < 3 ) {
            ...
            if (!empty($missingConfections)) {
                return response()->json(['message' => 'Il manque des catégories de confection', 'data'=>$missingConfections]); 
            }
            

            foreach ($request['articles'] as $confectionItem) {
